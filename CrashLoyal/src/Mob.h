...
protected: 
	...
	std::vector<std::shared_ptr<Attackable>> checkCollision();

	void processCollision(std::vector<std::shared_ptr<Attackable>> otherObjs, double elapsedTime);

	// A simple helper function that takes care of collisions with the river to avoid needing to represent it with objects.
	void riverCollisions();
